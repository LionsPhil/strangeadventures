...
int32 use_vacuum_collapser(char *title)
{
	...
	sprintf(texty, "%s", textstring[STR_LVC_ASKWHEN]);
		...
		if (t > t0 || upd == 1)
		{
			...
			if (survive)
			{
				if (sm_fleets[flt].num_ships>1)
					// Format specifier in textstring
					sprintf(texty, textstring[STR_PROBE_FLEET1], sm_fleets[flt].num_ships);
				else
					sprintf(texty, "%s", textstring[STR_PROBE_FLEET2]);
			}
			else
			{
				sprintf(texty, "%s", textstring[STR_PROBE_FLEET3]);
			}
...
//	Stop_All_Sounds();
	...
	sprintf(name, "%s", textstring[STR_PROBE_MISCDATA]);
	sprintf(texty, "%s", textstring[STR_PROBE_MISCDATA1+tof]);
	...
	else if (ecards[c].type == card_ally)
	{
		...
		if (it == race_none)
			sprintf(texty, "%s", textstring[STR_PROBE_MISCDATA3+tof]);
#ifndef DEMO_VERSION
		if (it == race_muktian)
			sprintf(texty, "%s", textstring[STR_PROBE_MISCDATA4+tof]);
#endif
	}
	else if ((ecards[c].type == card_item) || (ecards[c].type == card_rareitem) || (ecards[c].type == card_lifeform))
	{
		...
		if (ecards[c].type == card_lifeform)
			sprintf(texty, "%s", textstring[STR_PROBE_MISCDATA5+tof]);

		if (itemtypes[it].type == item_treasure)
			sprintf(texty, "%s", textstring[STR_PROBE_MISCDATA6+tof]);
	}
