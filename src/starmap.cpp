...
//											end = 1; player.death = 3;
			...
			if (player.engage)
			{
				...
				if ((shiptypes[0].engine==-1 || shiptypes[0].sysdmg[shiptypes[0].sys_eng]) && !player.fold)
				{
					...
					{
						sprintf(topic, "%s", textstring[STR_DRIVE_MISSING]);
						sprintf(texty, "%s", textstring[STR_DRIVE_MISSING2]);
					}
					else
					{
						sprintf(topic, "%s", textstring[STR_DRIVE_BROKEN]);
						sprintf(texty, "%s", textstring[STR_DRIVE_BROKEN2]);
					}
...
//	sprintf(lne, "%s", player.captname, 60-strlen(player.captname));
	...
	sprintf(top, "%s", textstring[STR_STARMAP_CARGO]);
	interface_drawborder(screen,
...
//		ik_print(screen, font_6x8, SM_INV_X + 16, SM_INV_Y + 24 + c * 8, 0, "%d", player.items[c]);
	...
	*/
	...
	sprintf(top, "%s", textstring[STR_STARMAP_SELECT]);
	if (player.target > -1)
	{
		if (sm_stars[player.target].explored)
			sprintf(top, "%s", sm_stars[player.target].planetname);
		else
			sprintf(top, "%s", sm_stars[player.target].starname);
	}
	interface_drawborder(screen,
